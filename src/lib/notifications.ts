...
export const emailTemplates = {
  newTask: (title: string, level?: string) =>
    ...
    <div style="padding: 40px 30px;">
      ...
      ${
        level
          ? `<div style="display: inline-block; background: #F3F0F9; color: #6344A6; padding: 8px 16px; border-radius: 50px; font-weight: bold; font-size: 14px; margin-bottom: 30px;">Nivel: ${level}</div>`
          : ""
      }
  ...
  newCommunityPost: (studentName: string, authorName: string, postTitle: string) =>
    baseTemplate(`
...
// ✅ FUNCIÓN DE ENVÍO DE NOTIFICACIÓN DE TROFEO (Asegurada exportación)
export const sendTrophyEmailNotification = async (
  ...
  if (!studentEmail || studentEmail.includes("sin-email")) return;
