...
export const emailTemplates = {
  newTask: (title: string) => `<h1>🚀 Nueva Misión: ${title}</h1><p>Entra a LuinGo para completarla.</p>`,
  graded: (title: string, grade: number) => `<h1>📝 Tarea Calificada: ${title}</h1><h2>Nota: ${grade}/10</h2>`,
  levelUp: (level: string) => `<h1>🎉 ¡Subiste de Nivel!</h1><p>Nuevo nivel: <strong>${level}</strong></p>`
};
